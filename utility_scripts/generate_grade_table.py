...
Writes a Markdown table to Pages/Syllabus_topics/summary-of-assignments.md.
...
def generate_grade_table():
    ...
    for mod_name, title, itype, due, pts in unique_rows:
        pts_display = fmt_pts(pts) if pts != "" else 0
        lines.append(f"| {mod_name} | {title} | {itype} | {due} | {pts_display} |")
        if pts != "":
            total += float(pts)
    ...
    out_path = os.path.join(out_dir, "summary-of-assignments.md")
